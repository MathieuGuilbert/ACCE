...
from ConstrainedClusteringViaPostProcessing_master.models.gurobi_import import *
from ConstrainedClusteringViaPostProcessing_master.models.utilities import *
...
def run_modified_model(n,k,p,ml,cl,label):
    # n, k: number of points, clusters
    # p is the similarity matrix  (If you inputing the distance matrix you need to *-1)
    # ml, cl are lists of index pair (for example: [(1, 3), (5, n)] )
    min_car = 1 #min_car = 1
    max_car = n #max_car = n
    clustering = np.argmax(p, axis=1)
    model = clustering_car_pw(n, k, p, ml, cl, min_car, max_car)
    ...
    per_change = 100.0 * per_change / n
    return metrics(label, clustering), metrics(label, partition), get_num_violates(ml, cl,clustering), per_change, model.Runtime, partition
